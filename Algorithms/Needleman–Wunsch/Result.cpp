...
void Back_Tracking( string X, string Y, int x, int y, string Res_x, string Res_y, string Result)
{
	...
	if( table[x][y] > table[x-1][y-1] && table[x][y] > table[x-1][y] && table[x][y] > table[x-1][y] )
	{
		Res_x+=X[x-1];
		Result+="|";
		Res_y+=Y[y-1];
	...
	else if( table[x-1][y] > table[x][y-1] && table[x-1][y] > table[x-1][y-1] )
	{
		Res_x+=X[x-1];
		Result+=" ";
		Res_y+="-";
