...
void Dy(char* X, char* Y, int i, int j, DyTable* Table)
{
	int m =0;
	int n =0;
	int match, indell, max_, temp, zero=0;
	
	int init1=0, init2=0;
	for ( m=0; m<=i; m++ ){
		Table->Data[m][0] = init1;
		init1 += IND;
	}

	for ( n=0; n<=j; n++ ){
		Table->Data[0][n] = init2;
		init2 +=IND;
	}
	...
	for ( m=1; m<=i; m++ )
	{
		for ( n=1; n<=j; n++ )
		{
			...
			temp = max(&match, &indell);
				
	        	indell = Table->Data[m-1][n] + IND;
			max_ = max(&temp, &indell);
...
void LCS_PrintTable( DyTable* Table, char* X, char* Y, int LEN_X, int LEN_Y ){
    int i = 0;
    int j = 0;
...
int dysearch(Chr* root, Chr* root2)
{
	...
	for(p1=root->next; p1; p1 = p1->next)
	{
		...
		for(p2=root2->next; p2; p2 = p2->next)
		{
			...
			LCS_PrintTable( &Table, p1->seq, p2->seq, xlen, ylen );
			Result2=0;
