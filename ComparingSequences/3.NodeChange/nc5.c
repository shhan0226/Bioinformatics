		...
		char* string, char* X1, char* Y1)
{
	...
	string[oldlen]='|';
	memset( &string[oldlen+1] ,0 ,sizeof(char)*MAX - oldlen -1);
	
	oldlen_x = strlen(X1);
	X1[oldlen_x] =X[m-1];
	memset( &X1[oldlen_x+1] ,0 ,sizeof(char)*MAX - oldlen_x -1);

	oldlen_y = strlen(Y1);
	Y1[oldlen_y] =Y[n-1];
	memset( &Y1[oldlen_y+1] ,0 ,sizeof(char)*MAX - oldlen_y -1);
...
int dysearch(Chr* root, Chr* root2)
{
	...
	for(p1=root->next; p1; p1 = p1->next)
	{
		for(p2=root2->next; p2; p2 = p2->next)
		{
			...
			A = (char*)malloc(sizeof(char)*clen+1);
			B = (char*)malloc(sizeof(char)*clen+1);
			C = (char*)malloc(sizeof(char)*clen+1);
			A1 = (char*)malloc(sizeof(char)*clen+1);
			B1 = (char*)malloc(sizeof(char)*clen+1);
			C1 = (char*)malloc(sizeof(char)*clen+1);
			memset(A, 0, sizeof(char)*clen+1);
			memset(B, 0, sizeof(char)*clen+1);
			memset(C, 0, sizeof(char)*clen+1);
			memset(A1, 0, sizeof(char)*clen+1);
			memset(B1, 0, sizeof(char)*clen+1);
			memset(C1, 0, sizeof(char)*clen+1);
